...
class PanicController {
    ...
    static async volumeButtonPanic(req, res) {
      ...
      `, [tripId]);
            ...
            const panicResult = await database_1.pool.query(`
    ...
    static async appButtonPanic(req, res) {
        const { tripId, coordinates } = req.body;
        const userId = req.user.id;
        try {
    ...
    static async getActivePanicAlerts(req, res) {
        try {
            const alertsResult = await database_1.pool.query(`
        SELECT pa.*, 
