...
class ReservationController {
    ...
    static async createReservation(req, res) {
        const { serviceTypeId, scheduledDate, scheduledTime, durationType, durationValue, originAddress, destinationAddress, originCoordinates, destinationCoordinates, specialRequirements } = req.body;
        const passengerId = req.user.id;
        try {
            // Obtener precio del tipo de servicio
            const serviceResult = await database_1.pool.query(`
      ...
      `, [serviceTypeId]);
            ...
            // Crear reserva
            const reservationResult = await database_1.pool.query(`
    ...
    static async getUserReservations(req, res) {
        ...
        try {
            const reservationsResult = await database_1.pool.query(`
    ...
    static async assignDriverToReservation(req, res) {
        const { reservationId } = req.params;
        const { driverId } = req.body;
        try {
            // Verificar disponibilidad del conductor en la fecha
            const conflictResult = await database_1.pool.query(`
