...
export class TrackingController {
    ...
    try {
      ...
      if (tripId) {
        ...
        if (tripResult.rows.length > 0) {
          ...
          if (targetUser) {
            io.to(`user_${targetUser}`).emit('location_update', {
              tripId,
              userId,
              coordinates,
              speed,
  ...
  static async getTripLocationHistory(req: Request, res: Response) {
  ...
  static async getNearbyDrivers(req: Request, res: Response) {
    ...
    try {
      ...
      const nearbyDrivers = driversResult.rows.filter(driver => {
        ...
        const latDiff = Math.abs(driverCoords.latitude - parseFloat(latitude as string));
        const lngDiff = Math.abs(driverCoords.longitude - parseFloat(longitude as string));
        const approximateDistance = Math.sqrt(latDiff * latDiff + lngDiff * lngDiff) * 111; // Aproximación en km
        
        return approximateDistance <= parseFloat(radiusKm as string);
      });
